...
The module automatically connects to the searchHub API to retrieve the necessary data, pre-built indices, and send back important statistical logs related to module performance and usage.
...
---------------------
For broad intent, smartSuggest recommends relevant or contextual scopes before search execution, preventing unfocused result pages and bounces.

4. Contextual Pre-Suggestions
-----------------------------
Intent often starts before typing. smartSuggest delivers pre-suggestions based on the current or last recent user context (e.g., category page, campaign entry, viewed product), with safe fallbacks to configured defaults managed in our UI  (Default Pre-Suggest Type) when context is limited or unknown.
